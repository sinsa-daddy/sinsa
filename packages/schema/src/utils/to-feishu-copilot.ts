...
export function toFeishuCopilotWithoutCreatedInfo(
  c: Omit<CopilotNextType, 'created_by' | 'created_time'>,
): Omit<FeishuCopilotType, 'created_by' | 'created_time'> {
  const val: Record<
    keyof Omit<FeishuCopilotType, 'created_by' | 'created_time'>,
    unknown
  > = {
    ...c,
    aurorian_1_id: c.aurorian_requirements[0].aurorian_id,
